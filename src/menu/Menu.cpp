...
Menu::Menu(Window* win,char* gameMode,int* amountOfPlayers,char* difficulty):win(win),gameMode(gameMode),amountOfPlayers(amountOfPlayers),difficulty(difficulty)
{
		...
		if(S1->col(mXH,mYH)&&keyDown=="MOUSEBUTTONDOWN")
		{
			*amountOfPlayers=1;
			...
			win->setBackground(11);
					win->drawLineUnder(DrawLineUnderMode,	DrawLineUnderNumPlayers,	DrawLineUnderDifficulty);
				return state;
		}
